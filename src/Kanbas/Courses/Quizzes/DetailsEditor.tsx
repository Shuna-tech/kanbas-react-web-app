...
export default function DetailsEditor() {
  ...
  useEffect(() => {
    const sumPoints = quiz.questions.reduce((acc, question) => acc + question.points, 0);
  ...
  const handleSaveAndPublish = async (quiz: any) => {
    try {
      ...
      //publish the quiz
      const publishResult = await client.publishQuiz(resultData._id || quiz._id);
      ...
      {activeTab === 'details' && (
        <div>
          ...
          <div className="row mb-3">
            ...
            <div className="col-10">
              <select id="wd-group" className="form-select" value={quiz.assignmentGroup} name="assignmentGroup" onChange={handleChange}>
                ...
                <div className="card-body">
                  ...
                  <div className="mb-3">
                    ...
                    <div className="input-group">
                      <input id="wd-due-date" type="date" className="form-control" value={quiz.dueDate}
                  ...
                  <div className="row">
                    <div className="col-md-6 mb-3">
                      ...
                      <div className="input-group">
                        <input id="wd-available-from" type="date" className="form-control" value={quiz.availableDate}
                          onChange={handleChange} name="availableDate" />
                      </div>
                    </div>
                    <div className="col-md-6 mb-3">
                      <label htmlFor="wd-available-until" className="form-label fw-bold">Until</label>
                      <div className="input-group">
                        <input id="wd-available-until" type="date" className="form-control" value={quiz.availableUntilDate}
                          onChange={handleChange} name="availableUntilDate" />
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
            ...
            <button className="btn btn-danger me-2" onClick={() => handleSave(quiz)}>Save</button>
            <button className="btn btn-primary" onClick={() => handleSaveAndPublish(quiz)}>Save and Publish</button>
          </div><br />
        </div>
      )}
      ...
      {activeTab === 'questions' && (
        <div>
          ...
          {quiz.questions.map((question) => (
            <div key={question._id} className="question-item" style={{ display: 'flex', alignItems: 'center' }}>
              <div style={{ display: 'flex', alignItems: 'center' }} >
                <input className='form-control me-2'
                  style={{ marginRight: '10px' }}
                  value={question.questionTitle}
                  onChange={(e) => handleQuestionTitleChange(question._id, e.target.value)} />
                <select
                  style={{ marginRight: '10px' }}
                  ...
                  <option value="Fill in Multiple Blanks">Fill in multiple blanks question</option>
                </select>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', marginLeft: '30px' }}>
                <Link to={getLinkPath(question.questionType)} key={question._id}>
                  <FaEdit className='fs-5 me-3' />
                </Link>
                <FaRegTrashAlt className='fs-5 me-3' onClick={() => deleteQuestion(question._id)} />
              </div><br /><br />
            </div>
          ))}
          <br />
          <div style={{ display: 'flex', justifyContent: 'center' }}>
            <button className="btn btn-primary" onClick={addNewQuestion}>+ New Question</button>
          </div><br />
          <div>
            <button className="btn btn-secondary me-3" onClick={handleCancel}>Cancel</button>
            <button className="btn btn-danger" onClick={() => handleSave(quiz)}>Save</button>
          </div>
